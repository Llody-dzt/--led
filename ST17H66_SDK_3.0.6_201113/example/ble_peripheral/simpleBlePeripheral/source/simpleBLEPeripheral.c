		...
		case LIGHT_POWER_MODEL:
			if(light_state)
			{
				if(model==0x0A)
				{
					...
					white_value = brightness* Color_temperature/100;
					yellow_value=brightness-white_value;			
					lxl_pwm_ctrl(PWM_CH0,white_value,PWM_CH1,yellow_value);
				}
				else if(model==0x0B)
				{
					lxl_pwm_ctrl(PWM_CH0,brightness,PWM_CH1,Color_temperature);
				}
			}
		break;
...
void lxl_pwm_on(void)
{
	hal_pwm_init(PWM_CH0, PWM_CLK_DIV_2, PWM_CNT_UP, PWM_POLARITY_FALLING);   //16M/8 = 2M
	hal_pwm_open_channel(PWM_CH0, P3);
	hal_pwm_init(PWM_CH1, PWM_CLK_DIV_2, PWM_CNT_UP, PWM_POLARITY_RISING);   //16M/8 = 2M
	hal_pwm_open_channel(PWM_CH1, P2);
	hal_pwm_start();	
}
void lxl_pwm_ctrl(uint8_t ch1,uint16_t value1, uint8_t ch2,uint16_t value2)
{
	if(value1==0){
		...
		hal_pwm_set_count_val(PWM_CH0, value1, 255);
		hal_pwm_set_count_val(PWM_CH1, value2, 255);
	}  
